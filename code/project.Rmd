...
# Plot Temperature
Temperature.ts = ts(data_clean$Temperature, start = c(2010,6), frequency = 52)
plot(Temperature.ts, ylab='Temperature')

# Plot Fuel_Price
Fuel.Price.ts = ts(data_clean$Fuel_Price, start = c(2010,6), frequency = 52)
plot(Fuel.Price.ts, ylab='Fuel_Price')
...
plot(Unemployment.ts, ylab='Unemployment')
...
autoplot(dWeeklySales.ts,ylab = 'WeeklySales_Diff(1)',main = 'Detrended Data')
...
```{r}
par(mfrow = c(1,2))
Acf(dWeeklySales.train,104, main = "Training Set (104 Lag)")
Pacf(dWeeklySales.train,104,main = "Training Set (104 Lag)")
par(mfrow = c(1,1))
```

```{r}
par(mfrow = c(1,2))
Acf(dWeeklySales.train,52, main = "Training Set (52 Lag)")
Pacf(dWeeklySales.train,52,main = "Training Set (52 Lag)")
par(mfrow = c(1,1))
```

According to the ACF and PACF plots, the ACF is cutting of and PACF is approximately tailing off within the first season.The ACF is cutting off after the first season. 
We will try SARMA(0,0,1)x(0,0,1) as our baseline model
...
autoplot(dWeeklySales.train, ylab = 'WeeklySales_Diff(1)',
main = 'Model: SARIMA(0,0,1)x(0,0,1)[52]')+
...
checkresiduals(m1)
```
For SARMA(0,0,1)x(0,0,1), the RMSE in training set is 69741.77 and RMSE in Test set is 52667.15.
...
autoplot(dWeeklySales.train, ylab = 'WeeklySales_Diff(1)',
main = 'Model: SARIMA(0,0,5)x(0,0,1)[52]')+
  autolayer(dWeeklySales.test, series = 'Actual')+
...
```
For SARMA(0,0,5)x(0,0,1), the RMSE in training set is 69741.77 and RMSE in Test set is 52667.15.
...
autoplot(dWeeklySales.train, ylab = 'WeeklySales_Diff(1)',
main = 'Model: SARIMA(0,0,4)x(0,0,1)[52]')+
  autolayer(dWeeklySales.test, series = 'Actual')+
...
```
For SARMA(0,0,4)x(0,0,1), the RMSE in training set is 76211.03 and RMSE in Test set is 49990.20. 


## Multiple Linear Regression

Step0: Add necessary columns
Add a column to create a dummy variable considering Blackfriday and Christmas periods.
```{r}
...
```
Add a column to create a dummy variable considering the low point after Christmas and before Thanksgiving.
```{r}
...
# All models are horrible, m6 should be the best.
```
...
# imply AR5
m6New = Arima(newdata.train[,1], order = c(5,0,0), xreg=newdata.train[,c(3,2,4,7,8)])
checkresiduals(m6New)

m6New.predict <- forecast(m6New, xreg = newdata.val[,c(3,2,4,7,8)], h = 38, level = 0)
accuracy(newdata.val[,1], m6New.predict$mean)
# it's worse than the original m6, so choose m6

autoplot(WeeklySales.ts, xlab= "Weekly Sales",series = 'Actual')+
  autolayer(m6$fitted, series = "Fitted")+
  autolayer(ts(m6p, start = c(2012,8), frequency = 52), series = 'Predicted')
```

Conclusion For Regression: m6 Model considering temperature, fuel price, holiday&x-mas&new year dummy will be the best multiple linear regression model.
