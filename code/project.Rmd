...
m7New = Arima(WeeklySales.ts, order = c(5,0,0), xreg=cbind(Temp.ts,Fuel.ts, HolidayFlag.ts, Cpi.ts, Xmas.ts, Ny.ts))
checkresiduals(m7New)

autoplot(WeeklySales.ts, series = 'Actual')+
  autolayer(m7New$fitted, series = 'Predicted')

accuracy(WeeklySales.ts, m7New$fitted)
```

Conclusion For Regression: m7New Model considering both autoregression and additional features performs best among all multiple linear regression.
