...
- q1: 在用trailing ma （align right）预测之前，是不是要用simple ma去预测一下test？（就是一根直线的，见week 06_bb.rmd）
- q2: 在rolling forward的时候，是不是可以把K=12调整一下，类似w=4或者6？
- q3: 修正了valid mean问题
...
ma.centered24 = ma(WeeklySales.ts, order = 24, centre = TRUE)
ma.centered48 = ma(WeeklySales.ts, order = 48, centre = TRUE)
#Plot Centered MA
...
ma24 = autoplot(WeeklySales.ts, xlab = "Time") +
  autolayer(ma.centered24, lwd = 1.5, show.legend = FALSE) +
  labs(title = "MA 24")

ma48 = autoplot(WeeklySales.ts, xlab = "Time") +
  autolayer(ma.centered48, lwd = 1.5, show.legend = FALSE) +
  labs(title = "MA 48")
...
```{r}
ma.trailing = rollmean(WeeklySales.ts, k = 12, align = "right")

autoplot(WeeklySales.ts, ylab = "Weekly Sales", xlab = "Time", col="Black") +
  autolayer(ma.centered12, series = "Moving Average", lwd = 1.5)+
...
ma.roll.ts = ts(ma.trail.pred.rolling,
                start = c(2010, 5+length(WeeklySales.ts) - nValid +1),
                frequency = 52)

valid.mean = ts(rep(mean(WeeklySales.ts), nValid),
                start = c(2010, 5+length(WeeklySales.ts) - nValid +1),
                frequency = 52)
```
```{r}
autoplot(WeeklySales.ts, ylab = "Weekly Sales") +
  ...
  autolayer(valid.mean, series = "Original Mean",lwd = 1.5) +
  autolayer(WeeklySales.test, series = "Test")+
  autolayer(ma.roll.ts, series = "Rolling", lwd = 1.5)
```
Accuracy
```{r}
ma.roll.ts

accuracy(WeeklySales.test, ma.roll.ts)
```
...
- q5: 选择后面那个会不会robust很多？+ residuals是不是会更合理一些？
- q6： arima和sarima里面的数字，尤其是seasonal part，会不会是以monthly计数的？（只是疑问） https://otexts.com/fpp2/arima-ets.html
```{r}
# SARMA(1,0,4)x(0,0,1)
...
m6New = Arima(WeeklySales.ts, order = c(5,0,0), xreg=cbind(Temp.ts,Fuel.ts, HolidayFlag.ts, Xmas.ts, Ny.ts))
checkresiduals(m6New)

autoplot(WeeklySales.ts, series = 'Actual')+
  autolayer(m6New$fitted, series = 'Predicted')

accuracy(WeeklySales.ts, m6New$fitted)
```
