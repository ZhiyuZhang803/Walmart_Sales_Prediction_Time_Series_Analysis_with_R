...
```{r}
WeeklySales.ts = ts(data_clean$Weekly_Sales, start = c(2010,2,5), frequency = 52)
plot(WeeklySales.ts)
```
...
```{r}
HolidayFlag.ts = ts(data_clean$Holiday_Flag, start = c(2010,2,5), frequency = 52)
Temp.ts = ts(data_clean$Temperature, start = c(2010,2,5), frequency = 52)
plot(Temp.ts)
Fuel.ts = ts(data_clean$Fuel_Price, start = c(2010,2,5), frequency = 52)
plot(Fuel.ts)
Cpi.ts = ts(data_clean$CPI, start = c(2010,2,5), frequency = 52)
plot(Cpi.ts)
Rate.ts = ts(data_clean$Unemployment, start = c(2010,2,5), frequency = 52)
plot(Rate.ts)
```
...
```{r}
library(ggplot2)
# WeeklySales.ts = ts(log(data_clean$Weekly_Sales), start = c(2010,2,5), frequency = 52)
